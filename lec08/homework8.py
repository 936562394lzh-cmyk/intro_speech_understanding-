...
def mstft_to_spectrogram(mstft):
    ...
    '''
    max_val = np.max(mstft)
    capped = np.maximum(0.001 * max_val, mstft)
    spectrogram = 20 * np.log10(capped)
    max_db = np.max(spectrogram)
    min_allowed = max_db - 60
    spectrogram = np.maximum(spectrogram, min_allowed)
    return spectrogram
